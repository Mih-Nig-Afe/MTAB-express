...
export default function Login() {
  ...
  const handleQuickFill = () => {
    ...
    setError('');
    toast.info('Autofilled demo administrator credentials', 'Demo Mode');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Welcome back, ${meRes.data.name}!`, 'Signed In');
      router.push('/dashboard');
    } catch (err: any) {
      const status = err.response?.status;
      const detail = err.response?.data?.detail;
      
      let errMsg = 'Login failed. Please check your phone and password.';
      if (status === 401) {
        errMsg = 'Incorrect phone number or password. Please try again.';
      } else if (detail) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 px-4 py-12">
      <div className="w-full max-w-md bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl p-8 border border-white/20">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Ethiopia&apos;s Inter-City Parcel Logistics</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="0911223344 or +2519..."
              />
            </div>
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-gray-100 flex flex-col items-center gap-3 text-center">
          ...
          >
            ⚡ Autofill Demo Admin (+251900000000)
          </button>
          <span className="text-xs text-gray-400">Default Password: admin123</span>
        </div>
      </div>
    </div>
  );
